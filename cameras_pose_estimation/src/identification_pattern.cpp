...
KDL::Chain LWR(){
  ...
  double pattern_length = 0.01735;
  double total_tool_length = pattern_length + 0.12597;//0.12597 m from joint to flange 
...
int main(int argc, char * argv[]){
	...
	// define the ros node
	ros::init(argc,argv, "identification_pattern");
	ros::NodeHandle nh_;
	ros::NodeHandle home("~");
	...
	while(ros::ok()){		
		if (initialized){
			...
			//broadcast the identification pattern's frame
			transform.setOrigin( tf::Vector3(cartpos.p[0], cartpos.p[1], cartpos.p[2]) );
			tf::Quaternion q;
			q.setRPY(roll, pitch, yaw);
			transform.setRotation(q);
			br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "kuka", "identification_pattern"));		
		}
		loop_rate.sleep();
		ros::spinOnce();
	}
